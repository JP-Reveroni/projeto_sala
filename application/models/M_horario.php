...
{
    /*
    Validação dos tipos de retorno nas validações dos codigos de erros
    0 - Erro de exceção
    ...
    */
    public function inserir($descricao, $horaInicial, $horaFinal) {
        try {
            //Verifico se o horario já esa cadastrado
    ...
    public function consultarHorario($codigo, $horaInicial, $horaFinal) {
        try {
            ...
            //Verifica se a consulta ocorreu com sucesspo
            if($retornoHorario->num_rows() > 0) {
                $linha = $retornoHorario->row();
                if(trim($linha->estatus) == "D") {
                    $dados = array(
                        'codigo' => 9,
                        'msg' => 'Horário desativado no sistema, caso precise reativar o mesmo, fale com o ADM.'
                    );
                }else {
                    $dados = array(
                        'codigo' => 10,
        ...
        //Envia o array $dados com as informações tratadas acima pela estrutura if
        return $dados;
    }
    ...
    public function consultar($codigo, $descricao, $horaInicial, $horaFinal) {
        ...
        //Envia o array dados com as informações tratadas acima pelo if
        return $dados;
    }

    public function alterar($codigo, $descricao,$horaInicial, $horaFinal) {
        try {
            //Verifica se o horario está cadastrado
            ...
            if ($retornoConsulta['codigo'] == 10) {
                ...
                //Executo a query de atualização
                $this->db->query($queryFinal);

                //Verifico se a atualização ocorreu com sucesso
                if($this->db->affected_rows() > 0) {
                    $dados = array(
                        'codigo' => 1,
    ...
    public function desativar($codigo) {
        try {
            //Verifico se o horario já está cadastrado
